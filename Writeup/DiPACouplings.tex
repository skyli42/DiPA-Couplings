...
\begin{proof}
    (Sketch) One path is $t_{true} t_{geq}^n$, where all transitions are assignments. There are other paths which are like $t_{init} t_{loop}^* t_{geq}$ that force the shift on $t_{geq}$ to be $1$ instead of $-1$ for a path independent coupling strategy. This causes the cost of the $t_{true} t_{geq}^n$ path to be $n^2$, but a coupling strategy that considers this path separately can have cost $n$. 
\end{proof}
