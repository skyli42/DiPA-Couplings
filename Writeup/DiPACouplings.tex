...
{\color{red} frame as a linear program?}
...
\begin{defn}
    A coupling strategy $C_A=(\bm{\gamma}, \bm{\gamma}')$ for a cyclic automata $A$ is \textbf{valid} if, for all paths $\rho$ of $A$, the following constraints hold for all adjacent input sequences $\alpha\sim\beta$ and for all $i$:
    \begin{itemize}
...
Note that given a complete path $\rho$ in a cyclic automaton $A$, a coupling strategy $C_A$ induces a coupling strategy $C_\rho$ for $\rho$.

\begin{defn}
    The \textbf{cost} of a coupling strategy $C_A = (\bm{\gamma}, \bm{\gamma}')$ for a cyclic automata $A$ is \begin{align*}
    cost(C_A) &= \sup_{\rho\in A}\max_{\texttt{in}\brangle{1}\sim \texttt{in}\brangle{2}} \sum_{i=0}^{n-1}(|-\texttt{in}_i\brangle{1}+\texttt{in}_i\brangle{2}-\gamma_i|)d_i+(|-\texttt{in}_i\brangle{1}+\texttt{in}_i\brangle{2}-\gamma_i'|)d_i'\\
    &=\sup_{\rho\in A}cost(C_\rho)
\end{align*}
    Further, let $G_A$ be the set of all valid coupling strategies for $A$. Then the \textbf{coupling cost} of $A$ is \[
        cost(A) = \min_{C_A\in G_A}cost(C_A)
    \]
\end{defn}

\begin{prop}
    Given a valid coupling strategy $C_A$ for a cyclic automaton $A$, $A$ is $cost(C_A)\varepsilon$-differentially private. 
\end{prop}
\begin{proof}
    Let $\rho$ be a complete path in $A$ and let $C_\rho$ be the coupling strategy for $\rho$ derived from $C_A$. We know from lemma \ref{multTransitionsCouplingProof} that, if $C_A$ is valid, then the lifting $\sigma\brangle{1}\{(a, b): a=\sigma\implies b=\sigma\}^{\#cost(C_\rho)\varepsilon}\sigma\brangle{2}$ is valid, 
    and therefore for all possible outputs $\sigma$ of $\rho$ and adjacent inputs $\texttt{in}\brangle{1}\sim\texttt{in}\brangle{2}$, $\forall \varepsilon>0, \PP[\rho, \sigma, \texttt{in}\brangle{1}]\leq e^{cost(C_\rho)\varepsilon}\PP[\rho, \sigma, \texttt{in}\brangle{2}]\leq e^{cost(C_A)\varepsilon}\PP[\rho, \sigma, \texttt{in}\brangle{2}]$.
\end{proof}
\begin{cor}
    A cyclic automaton $A$ is $cost(A)\varepsilon$-differentially private. 
\end{cor}
...
\begin{prop}
    \begin{align*}
        ...
        = \max_{\texttt{in}\brangle{1}-\texttt{in}\brangle{2}\in \{-1, 1\}} \sum_{i=0}^{n-1}(|-\texttt{in}_i\brangle{1}+\texttt{in}_i\brangle{2}-\gamma_i|)d_i+(|-\texttt{in}_i\brangle{1}+\texttt{in}_i\brangle{2}-\gamma_i'|)d_i'
    \end{align*}
\end{prop}
\begin{proof}
    Vishnu has something for this?
\end{proof}

\begin{lemma}
    For a cyclic automaton $A$, given adjacent input sequences $\texttt{in}\brangle{1}\sim \texttt{in}\brangle{2}$, a valid coupling strategy $C_A$ has finite cost $cost(C_A)<\infty$ if and only if the following constraint applies for all $i$:
    \begin{itemize}
...
\begin{proof}
    ...
    Let $T$ be the set of transitions $t_i$ in $A$ such that $t_i$ is \textbf{not} in a cycle. Note that $T$ is independent of any choice of path(s) through $A$. 

    Fix a complete path $\rho$ in $A$ and let $C_\rho$ be the coupling strategy for $\rho$ induced by $C_A$. 

    Let $D_\rho$ be the set of transitions $t_i$ in $\rho$ such that $t_i$ is in a cycle in $A$, i.e., $t_i\notin T$.  
...
\begin{lemma}
    If a cycle in $A$ contains an assignment transition, then there is no valid coupling strategy for $A$ such that the constraint holds. 
\end{lemma}

\begin{lemma}
    If [leaking pair], then there is no valid coupling strategy etc. 
\end{lemma}

\begin{lemma}
    If a cycle in $A$ contains a transition that outputs either $\texttt{insample}$ or $\texttt{insample}'$, then there is no valid coupling strategy for $A$ such that the constraint holds. 
\end{lemma}

\begin{lemma}
    If [privacy violating path], then there is no valid coupling strategy etc.
\end{lemma}

\begin{lemma}
    If there is no valid coupling strategy for $A$ such that the constraint holds, then [one of the four graph structures] must be present in $A$. 
\end{lemma}

\begin{thm}
    There is no valid coupling strategy with finite cost for $A$ if and only if [at least one of the four graph structures] are present in $A$.
\end{thm}

\subsubsection{Optimizing an Upper Bound}

- Solve the linear program subject to the constraints: 
...
\begin{thm}
    A DiPA $A$ is $d\varepsilon$-differentially private for some $d>0$ if and only if there exists some valid coupling strategy $S$ for $A$ such that $cost(S) = d$. 
\end{thm}
