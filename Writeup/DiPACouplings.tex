...
If we know a function's sensitivity, we can easily make it differentially private through the use of the \textbf{Laplace Mechanism}.
...
Approximate liftings are closely related to probabilistic couplings, which themselves are a commonly used tool in probabilistic analysis. We introduce couplings here as illustrative background.
...
In some sense, approximate liftings can be considered ``half-couplings'', where half of $\mu_L$ is coupled with half of $\mu_R$. Because of the close relationship between couplings and approximate liftings, we will often use ``couplings'' to generically refer to both approximate liftings and couplings in the technical sense.
...
\begin{defn}[Transition guards]
    $\mathcal{C}=\{\texttt{true}, \lguard[\texttt{x}], \gguard[\texttt{x}]\}$ is a set of \textbf{transition guards}.
\end{defn}
...
\begin{defn}[Valid inputs]
    ...
    \end{itemize}
    We will abuse notation and denote that $(\texttt{in}\brangle{1}, \texttt{in}\brangle{2})$ is a valid adjacent input pair by also writing $\texttt{in}\brangle{1}\sim\texttt{in}\brangle{2}$. In general, we will assume that all inputs are valid.
\end{defn}
