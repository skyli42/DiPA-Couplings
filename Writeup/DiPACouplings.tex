...
We introduce a new coupling possibility - between different 
...
\begin{defn}
    Let $L$ be a 2v-looping branch over a valid transition alphabet $\Sigma_T$. If, for a candidate coupling strategy $C_L = (\gamma, \gamma')$ for $L$, the following constraints are satisfied for all possible inputs $\texttt{in}\brangle{1}\sim\texttt{in}\brangle{2}$ and all $i$: \begin{enumerate}
...
\begin{lemma}
    If no coupling strategy satisfies the privacy constraint system, then there exists a leaking cycle, non-cancelling leaking pair, disclosing cycle, or privacy violating path in a variable in the looping branch.
\end{lemma}
\begin{proof}[\proofname~(sketch)]~\\
    ...
    - all that remains is to show that all leaking pairs must be non-cancelling, which follows pretty straightforwardly from the 
\end{proof}
