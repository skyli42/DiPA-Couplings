...
At each state, a real valued input $\texttt{in}$ is read in and Laplace noise sampled from the distribution $\Lap(0, \frac{1}{d\varepsilon})$ is added to it and stored in a temporary variable $\texttt{insample}$. Another temporary variable $\texttt{insample}'$ stores the sum of Laplace noise sampled from the distribution $\Lap(0, \frac{1}{d'\varepsilon})$ and $\texttt{in}$. 

Fundamentally, our program will transition between states based on a comparison between $\texttt{insample}$ and $\texttt{x}$ at each state. Additionally, our program will also output either some finite symbol from an alphabet $\Gamma$, $\texttt{insample}$, or $\texttt{insample}'$ at such a transition. 

More formally, we define a set of \textbf{guards} $\mathcal{C}=\{\texttt{true}, \lguard[\texttt{x}], \gguard[\texttt{x}]\}$. 

Then, given two states $q, q'$, a transition $t$ between them can be defined as $t = (q, q', c, \sigma)$ where $c\in \mathcal{C}$ and $\sigma\in \Gamma\cup\{\texttt{insample}, \texttt{insample}'\}$. 
