...
\begin{defn}
    Given a state space $Q$, a program $A$ is a set of complete executions $\rho$ such that for all $\rho=q_0\to\ldots\to q_n, \rho'=q'_0\to\ldots\to q'_m\in B$, $q_0=q'_0$ and $A$ satisfies the following closure property:\begin{itemize}
...
\begin{defn}
    For an execution $\rho$ of a program $A$, 
\end{defn}
...
\begin{proof}
    ...
    - either leaking pair or pvp

    \textbf{Case 1.1.2: Setting $\gamma_{at(i)}=1$ would violate (3) for $\gamma_{at(i)}$}
    assume not disclosing / leaking cycle

    - then privacy violating path

    \textbf{Case 1.1.3: Setting $\gamma_{at(i)}=1$ would violate (2) for some $\gamma_k$ such that $at(k)= at(i)$}
    ...
    In this case, attempting to resolve (1) by setting $\gamma_{at(i)} = 1$ must violate some other constraint. [gcycle or bad output]

    Additionally, attempting to resolve (1) by setting $\gamma_i = \gamma_{at(i)}$ must violate some other constraint. [lcycle?]
