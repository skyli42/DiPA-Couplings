...
Note (Vishnu): I will assume here 
...
\begin{lemma}\label{mvIndTransitionCoupling}
    ...
    The following four statements hold:\begin{enumerate}
        \item If $c = \lguard[\texttt{x}] \land \gguard[\texttt{y}]$, then $\forall \varepsilon>0$, the lifting $o\brangle{1}\{(a, b): a=\sigma\implies b=\sigma\}^{\#d\varepsilon}o\brangle{2}$ is valid for $d = $. 
        \item If $c = \gguard[\texttt{x}] \land \lguard[\texttt{y}]$, then $\forall \varepsilon>0$, the lifting $o\brangle{1}\{(a, b): a=\sigma\implies b=\sigma\}^{\#d\varepsilon}o\brangle{2}$ is valid for $d = $. 
...
\begin{proof}
    ...
    Then observe that if $\mu_x\brangle{1}\geq \mu_y\brangle{1}$, then \begin{align*}
