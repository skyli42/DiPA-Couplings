...
% \begin{prop}
%     The cost of a coupling strategy over a fixed path $\rho$ can be written as 
%     \begin{align*}
...
% \end{prop}
% \begin{proof}
%     Notice that the function $f: [-1, 1]^n \to \RR$ defined by 
%     \begin{align*}
%         f(\Delta) &= \sum_{i=0}^{n-1}(|\Delta-\gamma_i|)d_i+(|\Delta-\gamma_i'|)d_i'
%     \end{align*}
%     is a sum of absolute values of linear functions in $\Delta$, and so is a convex function of $n$ variables. Thus, the maximum of $f$ over $[-1, 1]^n$ occurs at one of the vertices of $[-1, 1]^n$. TODO: Complete if necessary
% \end{proof}
...
\begin{prop}
    Let $L$ be a looping branch with $n$ transitions. If $C = (\gamma, \gamma')$ is a coupling strategy for $L$ that satisfies the privacy constraint system, then
    \[cost(C) = \max_{\texttt{in}\brangle{1}\sim \texttt{in}\brangle{2}}\sum_{i=1}^{n}(|-\texttt{in}_i\brangle{1}+\texttt{in}_i\brangle{2}-\gamma_i(\texttt{in}_i\brangle{1}, \texttt{in}_i\brangle{2})|d_i+|-\texttt{in}_i\brangle{1}+\texttt{in}_i\brangle{2}-\gamma_i'(\texttt{in}_i\brangle{1}, \texttt{in}_i\brangle{2})|d_i')\]
\end{prop}

\begin{proof}
    TODO: (Vishnu) Essentially we can forget about paths and just sum over transitions
\end{proof}
