...
\begin{lemma}
    A coupling strategy $C_A=(\bm{\gamma}, \bm{\gamma}')$ for a cyclic automata $A$ is \textbf{valid} if and only if the following constraints apply:
    \begin{itemize}
        ...
        \item If $\sigma_i = \texttt{insample}'$, then $\gamma_i'=0$
    \end{itemize}
    Further, given adjacent input sequences $\alpha\sim \beta$, such a valid coupling strategy $C_A$ has finite cost $d<\infty$ if and only if the following constraints apply for all $i$:
    \begin{itemize}
        \item If $t_i$ is in a cycle, then $\gamma_i = -\alpha_i+\beta_i$. 
        \item If $t_i$ is in a cycle, then $\sigma_i \in \Gamma$. 
    \end{itemize}
\end{lemma}
