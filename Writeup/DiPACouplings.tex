...
These transitions are primarily \textbf{structural} elements, describing in general how a program can move from state $q$ to $q'$. However, we will also want to look at \textbf{individual paths} of a transition, with concrete values for $\texttt{x}$. 
...
\begin{defn}[is this a definition?]
    Let $t = (q, q', c, \sigma, \tau)$ be a transition and let $o$ be a possible output of $t$ (i.e. if $\sigma \in \Gamma$, let $s\in \Gamma$ and if $\sigma \in \{\texttt{insample}, \texttt{insample}'\}$, let $o\subseteq \RR$ (measurable)).
...
\begin{defn}[Valid inputs]
    ...
    \end{itemize}
    We will abuse notation and denote that $(\texttt{in}\brangle{1}, \texttt{in}\brangle{2})$ is a valid adjacent input pair by also writing $\texttt{in}\brangle{1}\sim\texttt{in}\brangle{2}$. 
\end{defn}
...
\begin{defn}[$d\varepsilon$-differential privacy for a transition]
    ...
    Then, given $X\brangle{1}, X\brangle{2}$, a transition $t=(q, q', c, \sigma, \tau)$ is \textbf{$d\varepsilon$-differentially private} for some $d>0$ if $\forall \varepsilon> 0$, for all valid adjacent input pairs $\texttt{in}\brangle{1}, \texttt{in}\brangle{2}$ and possible outputs $\sigma$ of $t$, $\PP[X\brangle{1}, t, \texttt{in}\brangle{1}, \sigma]\leq e^{d\varepsilon}\PP[X\brangle{2}, t, \texttt{in}\brangle{2}, \sigma]$. 
\end{defn}
...
\begin{lemma}\label{indTransitionCoupling}
    ...
    Then $\forall \varepsilon>0$ and for all $\gamma_x, \gamma_q, \gamma_q'\in [-1, 1]$ that satisfy the constraints \[
        ...
        \end{cases},
      \]
      the lifting $o\brangle{1}\{(a, b): a=\sigma\implies b=\sigma\}^{\#d\varepsilon}o\brangle{2}$ is valid for $d = (|\mu\brangle{1}-\mu\brangle{2}+\gamma_x|)\hat{d_q}+(|-\texttt{in}\brangle{1}+\texttt{in}\brangle{2}-\gamma_q|)d_q+(|-\texttt{in}\brangle{1}+\texttt{in}\brangle{2}-\gamma_q'|)d_q'$, and therefore $t$ is $d\varepsilon$-differentially private. 
\end{lemma}
...
We can first create the lifting $X\brangle{1}+\gamma_x (=)^{\#(|\hat{\mu_q}\brangle{1}-\hat{\mu_q}\brangle{2}+\gamma_x|)\hat{d_q}\varepsilon}X\brangle{2}$. This is analogous to shifting the threshold values that we want to compare inputs to in an algorithm like SVT. 
...
Thus, given the constraints \[
  ...
  \end{cases},
\]
we have shown that the lifting $o\brangle{1}\{(a, b): a=\sigma\implies b=\sigma\}^{\#d\varepsilon}o\brangle{2}$ is valid, where the cost $d = (|\hat{\mu_q}\brangle{1}-\hat{\mu_q}\brangle{2}+\gamma_x|)\hat{d_q}+(|-\texttt{in}\brangle{1}+\texttt{in}\brangle{2}-\gamma_q|)d_q+(|-\texttt{in}\brangle{1}+\texttt{in}\brangle{2}-\gamma_q'|)d_q'$. 
...
We start with \textit{paths}, comprised of a sequence of transitions. 

\begin{defn}[Program paths]
    Consider some set of program states $Q$. A program \textbf{path} is a sequence of transitions $t_0\cdot t_1\cdot \ldots\cdot t_{n-1}$ such that for all $i\in 0\ldots n-1$, $t_i = (q_i, q_{i+1}, c_i, \sigma_i, \tau_i)$ for some $c_i, \sigma_i, \tau_i$. We will often notate an path $\rho$ as $\rho = q_0\to q_1\to \ldots \to q_n$. 
...
As with transitions, we can discuss a specific instance, or execution, of a path with explicit values for $\texttt{x}$.

\begin{defn}[Program execution]
    A program \textbf{execution} is a sequence of instantiated transitions $\hat{\rho} = \hat{t}_0\cdot \hat{t}_1\cdot \ldots\cdot \hat{t}_{n-1}$ such that for all $i\in 0\ldots n-1$, where $\hat{t}_i = ((q_i, x_i), (q_{i+1}, x_{i+1}), c_i, \sigma_i, \tau_i)$, \begin{itemize}
        \item If $\tau_i = 0$, then $x_{i+1} = x_i$
        \item If $\tau_i = 1$, then $x_{i+1} = \texttt{insample}_i$, where $\texttt{insample}_i$ is a random variable representing a noised version of the input variable read in at $t_i$.
    \end{itemize}
\end{defn}

Observe that if $\rho$ is a complete path, then $x_0$ in $\hat{\rho}$ is functionally irrelevant.
...
\begin{defn}[Path Probabilities]
    Let $\rho = t_0\cdot t_1\cdot \ldots\cdot t_{n-1}$ be a path where $t_i = (q_i, q_{i+1}, c_i, \sigma_i, \tau_i)$ and let $o$ be a possible output of $\rho$. In particular, this means that for all $i$, if $\sigma_i \in \Gamma$, then $o_i = \sigma_i$, and if $o_i\in \{\texttt{insample}, \texttt{insample}'\}$, then $o_i \subseteq \RR$ (measurable). 

    Then let $\PP[x, \rho, \texttt{in}, o]$ be the \textbf{probability} that $t$ outputs (values in) $o$ when reading in input $\texttt{in} \in \RR^n$ given that $\texttt{x}_0 = x$.

    Similarly, let $\hat{\rho} = \hat{t}_0\cdot \hat{t}_1\cdot \ldots\cdot \hat{t}_{n-1}$ be an instantiated transition and let $o$ be a possible output of $\hat{\rho}$. 

    Then $\PP[\hat{\rho}, \texttt{in}, o]$ is the \textbf{probability} that $\hat{t}$ outputs $o$ when reading in input $\texttt{in} \in \RR^n$.
\end{defn}

\begin{defn}
    Given a path $\rho = t_0\cdot t_1\cdot \ldots\cdot t_{n-1}$, the \textbf{tail} of $\rho$ is defined as $tail(\rho) = t_1\cdot \ldots\cdot t_{n-1}$. Similarly, the tail of a path execution $\hat{\rho}=\hat{t}_0\cdot \hat{t}_1\cdot \ldots\cdot \hat{t}_{n-1}$ is $tail(\hat{\rho}) = \hat{t}_1\cdot \ldots\cdot \hat{t}_{n-1}$
...
\begin{obs}[Path Probabilities]
    Let $\hat{\rho} = \hat{t}_0\cdot \hat{t}_1\cdot \ldots\cdot \hat{t}_{n-1}$ be a path execution and let $o$ be a possible output of $\hat{\rho}$. Then the probability of $\hat{\rho}$ outputting $o$ can be defined recursively.

    So $\PP[\hat{\rho}, \texttt{in}, o] = \begin{cases}
        1 & |\hat{\rho}| = 0\\
        \PP[\hat{t}_0, \texttt{in}_0, o_0]\PP[tail(\hat{\rho}), tail(\texttt{in}), tail(o)|x_1]& |\hat{\rho}|>0
    \end{cases}$


    Now consider a complete path $\rho = t_0\cdot t_1\cdot \ldots\cdot t_{n-1}$. 
    
    Let $\hat{t}_{z, z'}$ be the instantiated transition $\hat{t}_x = ((q, z), (q, z'), c, \sigma, \tau)$.
    Then the probability that $t$ outputs $\sigma$ is \[\PP[t, X, \texttt{in}, o] = \begin{cases}   
        \int_{-\infty}^\infty\PP[X=z]\PP[\hat{t}_{z, z}, \texttt{in}, o]dz & \tau=0\\
        \int_{-\infty}^\infty\int_{-\infty}^\infty\PP[\texttt{insample} = z']\PP[X=z]\PP[\hat{t}_{z, z'}, \texttt{in}, o| \texttt{insample}=z']dz'dz & \tau=1
    \end{cases}\]
...
\begin{defn}[Branching program]
    Given a set of program states $Q$, a branching program $B$ is a finite set of complete paths over $Q$ such that for all $\rho=q_0\to\ldots\to q_n, \rho'=q'_0\to\ldots\to q'_m\in B$, $q_0=q'_0$. $B$ must also satisfy the following properties: \begin{enumerate}
        \item If any transition $t$ in any path $\rho\in B$ is of the form $(q, q', c, \sigma, \tau)$, then no other transitions of the form $(q, q^*, c, \sigma', \tau')$ for $q, q', q^*\in Q$ exist in any path of $B$. 
        Additionally, if any transition $t$ in any path $\rho\in B$ is of the form $(q, q', \texttt{true}, \sigma, \tau)$, then transitions of the form $(q, q^*, \lguard[\texttt{x}], \sigma', \tau')$ or $(q, q^*, \lguard[\texttt{x}], \sigma', \tau')$ do not exist in any path of $B$.
...
\begin{prop}
    A branching program $B$ is $d\varepsilon$-differentially private for $d>0$ if and only if for every complete path $\rho$ in $B$, $\rho$ is $d\varepsilon$-differentially private.
\end{prop}
\begin{proof}
    From the conditions of output distinction and determinism, note that for every single possible output $\sigma$ of $B$, there is exactly one possible path $\rho$ that could have output $\sigma$. Thus, the probability that $B$ outputs $\sigma$ is exactly the probability that $\rho$ outputs $\sigma$; the result follows immediately.
\end{proof}
...
\begin{defn}[Lassos]
    Consider a complete path $\rho = q_0\to\ldots \to q_n$ such that for some $i<n$, $q_i = q_n$. Suppose also that $\forall j\neq i$, $q_j \neq q_n$. A lasso $L_\rho$ is the infinite set of paths $\{q_0\to\ldots\to q_{i-1}\to (q_i\to\ldots\to q_{n-1})^k\to q_n| k\in \NN\}$, where $(q_i\to\ldots\to q_{n-1})^k$ means that the subpath $(q_i\to\ldots\to q_{n-1})^k$ is iterated $k$ times. 
...
\begin{defn}[Lasso privacy]
    A lasso $L_\rho$ is $d\varepsilon$-differentially private for $d>0$ if for all complete paths $\rho'$ in $L_\rho$, for all possible outputs $\sigma_{\rho'}$ of $\rho'$ and input sequences $\texttt{in}\brangle{1}\sim \texttt{in}\brangle{2}$, $\forall \varepsilon>0, \PP[\rho', \sigma_{\rho'}, \texttt{in}\brangle{1}]\leq e^{d\varepsilon}\PP[\rho', \sigma_{\rho'}, \texttt{in}\brangle{2}]$.
\end{defn}

\begin{prop}
    A lasso $L_\rho$ is $d\varepsilon$-differentially private for $d>0$ if and only if for all complete paths $\rho'$ in $L_\rho$, $\rho'$ is $d\varepsilon$-differentially private. 
\end{prop}
...
\begin{defn}[Validity]
    A coupling strategy $C_{L_\rho}=(\bm{\gamma}, \bm{\gamma}')$ for a lasso $L_\rho$ is \textbf{valid} if, for all paths $\rho'$ of $A$, the following constraints hold for all adjacent input sequences $\alpha\sim\beta$ and for all $i$:
    \begin{itemize}
...
Note that given a complete path $\rho'$ in a lasso $L_\rho$, a coupling strategy $C_A$ induces a coupling strategy $C_{\rho'}$ for $\rho'$.
...
\begin{defn}
    Given a finite state space $Q$, a program $A$ is a set of complete paths $\rho$ such that for all $\rho=q_0\to\ldots\to q_n, \rho'=q'_0\to\ldots\to q'_m\in B$, $q_0=q'_0$ and $A$ satisfies the following closure property:\begin{itemize}
        \item If a complete path $\rho\in A$ such that $\rho = q_0\to \ldots \to q_i \to \ldots \to q_j \to q_i\to \ldots \to q_n$, then, for all $k\in \NN$, the path $\rho_k\in A$, where $\rho_k = q_0\to \ldots \to (q_i \to \ldots \to q_j)^k \to q_i\to \ldots \to q_n$.
    \end{itemize}
    Additionally, $A$ must satisfy the properties of \textbf{determinism} and \textbf{output distinction}, as with branching programs. 
\end{defn}

Intuitively, this closure condition means that if a path $\rho$ with the cycle $q_i\to \ldots \to q_j\to q_i$ is in $A$, then every path that is identical to $\rho$ except with the cycle iterated a different number of times must be in $A$ as well. 
...
As with branching programs, the conditions of determinism and output distinction mean that knowing the output of a program means that we know what path the program must have taken. 

\begin{prop}
    For every complete path $\rho$ of a program $A$, $\rho$ is $d\varepsilon$-differentially private if and only if $A$ is differentially private. 
\end{prop}

As expected, then, we can use coupling strategies for paths to prove privacy.

\begin{prop}
    If, for every complete path $\rho$ of a program $A$, there is a coupling strategy $C_\rho$ such that $cost(C_\rho)<d$, then $A$ is $d\varepsilon$-differentially private.
\end{prop}

However, this naive approach is not actually tractable, since there are possibly an unbounded number of paths in a single program $A$. 

What we will show instead is that, inspired by our work with lassos, we can choose a coupling strategy for each of a finite number of path classes of $A$, which will induce coupling strategies for every path of $A$. 

\hrulefill
{\color{red} ALTERNATIVE DEFINITION FROM SUBGRAPHS}
\begin{defn}
    For an path $\rho$ of a program $A$, let $graph(\rho)$ be the subgraph of the underlying graph of $A$ that is traversed by $\rho$. That is, if the underlying graph of $A$ is $G=(V, E)$, then $graph(\rho) = (V', E')$, where $V'\subseteq V$, $E'\subseteq E$, and for all transitions $t_i = (q_i, q_{i+1}, c_i, \sigma_i, \tau_i)$, $q_i, q_{i+1}\in V$ and $(q_i, q_{i+1}) \in E$. 
\end{defn}

Note that the relation between complete paths $\sim_G$ where $\rho \sim_G\rho'$ if and only if $graph(\rho) = graph(\rho')$ is clearly an equivalence relationship; let $[\rho]_G$ be the equivalence class of $\rho$ defined by this relationship for any complete path $\rho$ of a program $A$. 

\begin{defn}[Class Coupling Strategies]
    Consider an path class $[\rho]_G$ of a program $A$ and let $G=(V, E) = graph(\rho)$ be the underlying subgraph of $[\rho]_G$. A [name tentative] class coupling strategy $C_{[\rho]_G}$ for $[\rho]_G$ is a function $C_{[\rho]_G}: E\to (\RR\times\RR\to[-1, 1])\times (\RR\times\RR\to[-1, 1])$ mapping edges of $G$ to shift functions $(\gamma, \gamma')$.
\end{defn}

Observe that there are a finite possible number of path classes of a program $A$ since $A$ must have a finite number of states; additionally, a class coupling strategy for an path class $[\rho]_G$ directly induces path coupling strategies for every complete path in the class. 


\hrulefill

\begin{defn}[path Descriptions]
    Given a complete path $\rho = q_0\to \ldots \to q_n$ of a program $A$, let the description $desc(\rho)$ of $\rho$ be the cycle-free substring of $\rho$. {\color{red} still don't like this definition}
\end{defn}

Note that the relation between complete paths $\sim$ where $\rho \sim\rho'$ if and only if $desc(\rho) = desc(\rho')$ is clearly an equivalence relationship; let $[\rho]$ be the equivalence class of $\rho$ defined by this relationship for any complete path $\rho$ of a program $A$. 

\begin{defn}[path Class]
    An path class $[\rho]$ of a program $A$ is the equivalence class of an path $\rho$ of $A$ under the aforementioned equivalence relation. 
\end{defn}

\begin{obs}
    There are a finite number of path classes of a program $A$ because there are a finite number of simple paths through any finite graph.
\end{obs}

\begin{defn}[Class Coupling Strategies]
    Given an path class $[\rho]$ of a program $A$, let $T$ be the set of all transitions of any path in $[\rho]$. Note that $T$ is finite. Then a (class) coupling strategy for $[\rho]$ is a pair of functions $C = (\gamma, \gamma')$ such that $\gamma:T\to (\RR\times\RR\to[-1, 1])$ and $\gamma':(\RR\times\RR\to[-1, 1])$ are two shift functions that map each transition in $[\rho]$ to a shift in $[-1, 1]$. 
\end{defn}


\begin{defn}[Induced Coupling Strategy]
    {\color{red}not sure how necessary this is}
    Given a class coupling strategy $C = (\gamma, \gamma')$ for an path class $[\rho]$ and a specific path $\rho\in [\rho]$, the coupling strategy for $\rho=q_0\to \ldots \to q_n$ induced by $C$ is the pair of functions $\gamma_\rho, \gamma'_\rho$ such that 
    \begin{align*}
...
\begin{defn}
    The cost of a class coupling strategy $C$ for an path class $[\rho]$ is $cost(C) = \sup_{\rho\in [\rho]}cost(C_{\rho})$, where $C_{\rho}$ is the path coupling strategy for $\rho'$ induced by $C$.
\end{defn}

\begin{prop}
    If, for every complete path class $[\rho]$ of a program $A$, there is a coupling strategy $C_{[\rho]}$ such that $cost(C_{[\rho]})<d$, then $A$ is $d\varepsilon$-differentially private.
\end{prop}

As with lassos, we primarily care about making sure that cycle transitions are costless. 

\begin{prop}
    A valid class coupling strategy $C=(\gamma, \gamma')$ for an path class $[\rho]$ has finite coupling cost if and only if the following constraint holds for all $i$: 
    \begin{itemize}
...
Indeed, collapsing the infinite collection of paths in a program to the finite set of path classes does not cost us anything. 
...
We are primarily(?) concerned with the boolean question of privacy; that is, whether or not there exists \textit{any} finite $d>0$ such that a program is $d\varepsilon$-differentially private. 

Perhaps surprisingly, we show that coupling proofs are \textbf{complete} for programs of this form; while producing a valid and finite cost coupling proof is clearly sufficient for a program to be private, it is also necessary for such a coupling proof to exist for a program to be differentially private. 

\begin{lemma}
    If there exists a valid class coupling strategy with cost $d>0$ for a program $P$, then $P$ is $d\varepsilon$-differentially private. 
\end{lemma}
\begin{proof}
    Follows from the construction of coupling strategies. 
\end{proof}

\begin{cor}\label{programFiniteCostCor}
    A program $P$ is $d\varepsilon$-differentially private for some $d<\infty$ if, for all path classes $[\rho]$ of $P$, there exists a valid class coupling strategy $C_{[\rho]} = (\gamma, \gamma')$ such that the following constraint holds for all $i$:
    \begin{itemize}
        \item If $t_i$ is in a cycle in $[\rho]$ and $c_i\neq\texttt{true}$, then $\gamma_i = -\texttt{in}\brangle{1}_i+\texttt{in}\brangle{2}_i$ and $\gamma_i' = -\texttt{in}\brangle{1}_i+\texttt{in}\brangle{2}_i$.
    \end{itemize}
\end{cor}
...
\begin{defn}[Privacy Constraint System]
    Consider a program $P$ and path class $[\rho]$ of $P$. If, for a candidate class coupling strategy $C = (\gamma, \gamma')$ for $[\rho]$, the following constraints are satisfied for all $i$: \begin{enumerate}
        \item If $c_i = \lguard[\texttt{x}]$, then $\gamma_i\leq\gamma_{at(i)}$
        \item If $c_i = \gguard[\texttt{x}]$, then $\gamma_i\geq\gamma_{at(i)}$
        \item If $\sigma_i = \texttt{insample}$, then $\gamma_i=0$
        \item If $\sigma_i = \texttt{insample}'$, then $\gamma_i'=0$
...
\begin{lemma}
    If, for every path class $[\rho]$ in a program $P$, there exists a class coupling strategy $C_{[\rho]}$ that satisfies the privacy constraint system, then there exists some finite $d>0$ such that $P$ is $d\varepsilon$-differentially private.    
\end{lemma}
\begin{proof}
    Follows immediately from corollary \ref{programFiniteCostCor}.
\end{proof}

\begin{lemma}\label{ProgramCounterexampleLemma}
    If for some path class $[\rho]$ in a program $P$ there does not exist a class coupling strategy $C_{[\rho]}$ that satisfies the privacy constraint system, then there does not exist any finite $d>0$ such that $P$ is $d\varepsilon$-differentially private.
\end{lemma}
...
\begin{defn}[\cite{chadhaLinearTimeDecidability2021}]
    A Differentially Private Automaton (DiPA) $A$ is a 7-tuple $(Q, \Sigma, C, \Gamma, q_{init}, X, P, \delta)$ where
    \begin{itemize}
...
\begin{defn}
    A path $\rho = q_0\to\ldots\to q_n$ in $A$ is complete if $q_0 = q_{init}$. 
\end{defn}

\begin{defn}
    Let $\rho$ be a path in a DiPA $A$, let $\texttt{in}$ be a valid input sequence and let $o$ be a possible output of $\rho$. In particular, if $o\in \{\texttt{insample}, \texttt{insample}'\}$, then we require that $o$ is an \textit{interval} $(a, b)\subseteq \RR$, rather than simply a measurable set as before. 
...
\begin{lemma}
    For every DiPA $A = (Q, \Sigma, C, \Gamma, q_{init}, X, P, \delta)$, there exists a program $P$ with state space $Q$ such that for every complete path $\rho$ in $A$, there exists a complete path $\rho'$ in $P$ such that for all possible outputs $o$ of $\rho$ and valid input sequences $\texttt{in}$, $\PP[\rho, \texttt{in}, o] = \PP[\rho', \texttt{in}, o]$
\end{lemma}
...
\begin{prop}
    For every DiPA $A = (Q, \Sigma, C, \Gamma, q_{init}, X, P, \delta)$, there exists a program $P$ with state space $Q$ such that $A$ is $d\varepsilon$-differentially private if and only if $P$ is $d\varepsilon$-differentially private. Conversely, for every program $P$ with state space $Q$, there exists a DiPA $A = (Q, \Sigma, C, \Gamma, q_{init}, X, P, \delta)$ such that $P$ is $d\varepsilon$-differentially private if and only if $A$ is $d\varepsilon$-differentially private.
\end{prop}

Similarly to programs, the privacy of any DiPA $A$ is completely characterized by four graph-theoretic structures. 
...
{\color{red} Move these definitions to appendix?}
...
\begin{thm}[optimistically]
    A GDiPA $A$ is $d\varepsilon$-differentially private if and only if, there exists a valid assignment of coupling strategies over all segments of $A$.
\end{thm}
