...
A program state is a program location coupled with a value for the program variables $\texttt{x}$, an input sequence $\texttt{in}$, and an output sequence $\sigma$. Let $S = Q\times\RR\times\RR^*\times (\Gamma\cup\{\texttt{insample}, \texttt{insample}'\})^*$ be the set of all possible program states. 

Then the semantics of a transition $t$ is a function $\llbracket t\rrbracket: S\to dist(S)$ that maps an initial program state to a distribution of subsequent program states following the expected semantics. For example, \[\llbracket(q_0, q_1, \lguard[\texttt{x}], \top, 1)\rrbracket((q_0, 0, \texttt{in}, \sigma)) = \begin{cases}
    (q_1, \texttt{insample}, \texttt{in}_{1\ldots}, \sigma\top)&\texttt{insample}<0\\
    (q_{term}, -, -, \sigma) & \texttt{insample}\geq 0
\end{cases},
\] 
where $\texttt{insample}\sim\Lap(\texttt{in}_0, \frac{1}{d_0\varepsilon})$. 

\hrulefill

\textbf{Another formulation}

Let $O\subset \PP(\RR)$ be the set of all measurable subsets of $\RR$. Let $t = (q, q', c, \sigma, \tau)$ be a transition, let $\texttt{in} \in \RR$ be a valid input for $t$, and let $o\in \Gamma\cup O$ be a possible output of $t$. Let $X \sim \Lap(\mu, \frac{1}{d_x\varepsilon})$ be a random variable sampled from a Laplace distribution. Let $P(q) = (d_q, d_q')$.

Together, these define a random process: if, starting with $\texttt{x} = X$, the guard $c$ is satisfied given input $\texttt{in}$ and $o$ is output. 

More precisely, let $I\sim \Lap(\texttt{in}, \frac{1}{d_q\varepsilon})$ and $I'\sim \Lap(\texttt{in}, \frac{1}{d_q'\varepsilon})$ be independent random variables.

For both $I$ and $I'$, given $o$, we say that $I$ (or $I'$, respectively) matches $o$ if $o\subseteq \RR$ and $I\in o$. 

Then let a ``success'' for $(X, t, \texttt{in}, o)$ be the event where $c$ is satisfied for $\texttt{x} = X$ and $\texttt{insample} = I$ and, if $\sigma = \texttt{insample}$ or $\sigma = \texttt{insample}'$, then $I$ matches $o$ or $I'$ matches $o$, respectively. 

We will notate the probability of such an event happening as $\PP[X, t, \texttt{in}, o]$
...
We can describe programs as Markov processes with state space $S = Q\times\RR\times\RR^*\times (\Gamma\cup\RR)^*$.

A state $s\in S$ is the tuple $(q, \texttt{x}, \texttt{in}, \sigma)$. If there exists a transition $t = (q, q', c, o, 0)$, then $s$ transitions to state $(q', \texttt{x}, \texttt{in}_{1:}, \sigma\cdot o)$ with probability equal to the probability that $c$ is satisfied given that $\texttt{x} = \texttt{x}$ and $\texttt{in}$ is the input. 

If $\texttt{in}$ is empty, then the Markov process terminates. 
...
\begin{proof}
    ...
    Assume that there exists an optimal coupling strategy $C$ that assigns the same coupling strategy to $t_{init}$ in both $\rho_1$ and $\rho_2$. If $\gamma_0(0, 1) \neq -1$ in $C_{\rho_1}$, then $cost(C_{\rho_1}) > 2$ in the same method as the above, a contradiction. Thus, $\gamma_0(0, 1) = -1$ in $C_{\rho_1}$, which by hypothesis, means that $\gamma_0(0, 1) = -1$ in $C_{\rho_2}$. We have the privacy constraint $\gamma_i \leq \gamma_0$ on $\rho_2$, which also means that $\gamma_i = -1$ identically for all $i > 0$. However, this means that 
    \begin{align*}
...
\end{prop}
\begin{proof}
    Let $\rho = q_0\to q_1\to\ldots \to q_n$ be a path in $A=(Q, \RR, C, \Gamma, q_0, X, P, \delta)$. 
...
\begin{prop}
    For all input sequences $\texttt{in}$ and possible outputs $\sigma$, and for all paths $\rho$ in a DiPA $A$, $\PP[\rho, \texttt{in}, \sigma] = \PP[\hat{\rho}, \texttt{in}, \sigma]$.
\end{prop}
\begin{proof}
...
\begin{defn}
    Define the approximate privacy cost of a looping branch $L$ to be the solution to the following optimization problem: 
    \begin{align*}
        approx(L) = \min_{\gamma, \gamma' \in [-1, 1]^n} &\max_{\Delta \in [-1, 1]^n} \sum_{i = 1}^n \left(|\Delta_i - \gamma_i| d_i + |\Delta_i - \gamma_i'|d_i' \right)\\ 
            \text{subject to } 
            &\ \gamma_{at(i)} \leq \gamma_i \text{ if } c_i = \lguard, \\
            &\ \gamma_{at(i)} \geq \gamma_i \text{ if } c_i = \gguard, \\
            &\ \gamma_i = 0 \text{ if } \sigma_i = \texttt{insample}, \\
            &\ \gamma_i' = 0 \text{ if } \sigma_i = \texttt{insample}'\\
            &\ \gamma_i = \gamma_i'= 1 \text{ if } t_i \text{ is in a cycle and has } c_i = \lguard,\\ 
            &\ \gamma_i = \gamma_i'= -1 \text{ if } t_i \text{ is in a cycle and has } c_i = \gguard
    \end{align*}
    which constructs a coupling proof of privacy in which $\gamma$ and $\gamma'$ are constant with respect to $\texttt{in}\brangle{1}$ and $\texttt{in}\brangle{2}$ on each transition. 
\end{defn}

\begin{prop}
    We are able to compute the inner maximum explicitly, and thus the approximate privacy cost can be written as a linear program. 
\end{prop}

\begin{prop}
    For a looping branch $L$ with $n$ distinct transitions, we have 
    \[cost(L) \leq approx(L) \leq cost(L) + \sum_{i = 1}^n (d_i + d_i')\]
\end{prop}
...
\subsection{Two-Threshold Programs: GDiPA}

\subsubsection{Multivariable Transitions}

{\color{red} notation for guards}
...
\begin{defn}[2v-transitions]
    A two-variable transition (2v-transition) is a tuple $(q, q', c, \sigma, \tau)$ where \begin{itemize}
        ...
        \item $c\in \{\texttt{true}, \lguard[\texttt{x}], \gguard[\texttt{x}]\}\times \{\land, \lor\}\times \{\texttt{true}, \lguard[\texttt{y}], \gguard[\texttt{y}]\}$ is a transition guard.
...
- combining independent couplings together

- new type of coupling

\subsubsection{Cross-Couplings{\color{red}CHANGE NAME}}

We introduce a new type of coupling strategy for a single (2v-)transition:



{\color{red} Valid coupling strategies}

- coupling strategy is a series of shifts - 
...
Paths:
{\color{red} sanity check that you can't derive contradictory inequalities}
...
\begin{lemma}
    If there does not exist a single variable coupling strategy with respect to a variable $\texttt{x}$ for a looping branch $\rho$ that satisfies the privacy constraint conditions for $\texttt{x}$, then $\rho$ must contain either a leaking cycle, leaking pair, disclosing cycle, or privacy violating path in $\texttt{x}$.
\end{lemma}
