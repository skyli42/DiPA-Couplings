...
We now begin the process of building up a program model through the lens of regular languages. Each program is parametrized by a privacy parameter $\varepsilon>0$. 
We model programs as simply regular languages comprised of possible program executions (which we call straight line programs) where each individual execution corresponds to a word in the language. This approach simultaneously allows us to build approximate liftings for each straight line program that prove the overall privacy of a program.
