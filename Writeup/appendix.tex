...
\begin{lemma}[Detailed version of \ref{simplifiedMultTransitionsCouplingProof}]\label{multTransitionsCouplingProof}
    Let $\rho = t_0\ldots t_{n-1}$ be a initialized path of length $n$. 
...
\sky{These have to be rewritten without locations + moved to appendix}
...
\begin{proof}
    ...
    For each $i$, construct also the trajectory $L_i = L\left(t_{true}^{(n + i)} \left(t_{loop}^{(i)}\right)^* t_{geq}^{(i)}\right)$ such that each transition $t_{geq}^{(i)}$ is preceded by an arbitrary number of transitions with guard $\lguard$.
    ...
    From the privacy constraints on the trajectories $L_i$, we see that $\gamma_{t_{true}^{n + i}}(1, 0) = 1$ from the same method as in \ref{costDependspathProp}, which then implies from the constraint $\gamma_{t_{true}^{n + i}} \leq \gamma_{t_{geq}^{(i)}}$ that $\gamma_{t_{geq}^{(i)}}(1, 0) = 1$. 
    ...
    For the trajectories $L_i$, the coupling strategy $C_{L_i}^* = (\gamma^*, \gamma^{'*})$ assigns
...
\begin{proof}[\proofname~of lemma \ref{finiteCostConstraintLemma}]
    ...
    Fix a initialized path $\rho\in L$ and let $C_\rho$ be the coupling strategy for $\rho$ induced by $C$. 
    ...
    Then there exists some initialized path $\rho$ in $L$ of the form $a(bt_ic)^*d$ for some $a, b, c, d\in \Sigma_T^*$.
    
    Let $\rho_k=a(bt_ic)^kd$ be the corresponding initialized path in $L$ with $(bt_ic)$ iterated $k$ times. This is equivalent to iterating the cycle containing $t_i$ $k$ times. Then for all $k\in \NN$, \begin{align*}
...
\begin{lemma}\label{cycleGammaConstraints}
    If a coupling strategy $C=(\mathbf{\gamma}, \mathbf{\gamma}')$ for a trajectory $L$ is valid and has finite cost, then the following must hold for all $i$:
    \begin{enumerate}
...
\begin{lemma}\label{unsatisfiableImpliesNotWellformedLemma}
    If a trajectory $L$ generated by $G_L$ satisfies output distinction and there does not exist a coupling strategy $C$ that satisfies the privacy constraint system for $L$, then $G_L$ must contain either a leaking cycle, a leaking pair, a disclosing cycle, or a privacy violating path. 
\end{lemma}
...
\textbf{The following claims help prove theorem \ref{LinearTimeDecidingPrograms}}
...
\begin{proof}[Proof of \ref{prop:paths_in_privacy_graph}]
    ...
    \begin{itemize}
        ...
        If $k = 1$, then the path $v_{i_0} \to v_{i_1}$ comprises of a single edge $(v_{i_0}, v_{i_1})$ in $G_P$. So, there is a trajectory $L$ for which $(v_{i_0}, v_{i_1}) \in G_L$, for which there is the privacy constraint $\gamma_{i_0} \leq \gamma_{i_1}$. 
        ...
        \begin{enumerate}
            ...
            Since we have the edge $(v_{i_{k - 1}}, v_{i_k})$ in $G_P$, there exists a trajectory $L$ for which $(v_{i_{k - 1}}, v_{i_k}) \in G_L$, for which there is the privacy constraint $\gamma_{i_{k - 1}} \leq \gamma_{i_k}$.
            ...
            Similar to the argument above, we either have that $t_{i_{k - 1}}$ precedes $t_{i_k}$, or $t_{i_k}$ precedes $t_{i_{k - 1}}$ in some trajectory. We cannot have that $t_{i_{k - 1}}$ precedes $t_{i_k}$, and so $c_{i_{k - 1}}$ is forced to be $\lguard$. We can then construct a path $\rho \in P$ such that $t_{i_k} \cdots t_{i_0}$ is a subsequence of $\rho$ with $guard(t_{i_j}) = \lguard$ for all $j \in \{k - 1, \dots, 0\}$. 
        \end{enumerate}
    \end{itemize}
...
\begin{cor}
    The path $v_{i_0} \to \dots \to v_{i_k}$ is in $G_P$ if and only if there is a trajectory $L$ in $P$ such that $v_{i_0} \to \dots \to v_{i_k}$ is a path in $G_L$.
\end{cor}

\begin{proof}[Proof of \ref{programPrivacyConstraintGraphPathReq}]
    There is a path from $\bf 1$ to $\bf -1$ in the privacy constraint graph of $P$ if and only if there is a path from $\bf 1$ to $\bf -1$ in the privacy constraint graph of some trajectory $L$ in $P$ by Proposition \ref{prop:paths_in_privacy_graph}. This is true if and only if there exists some $L$ which is not differentially private, which is true if and only if $P$ is not differentially private.
\end{proof}
