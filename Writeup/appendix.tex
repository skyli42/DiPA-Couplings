...
This section contains more precise versions of the major coupling lemmas in sections \ref{svTransitionsSec} and \ref{svSLPSection} as well as any omitted proofs. 
...
This section contains any omitted proofs and precise statements of claims in sections \ref{svProgramsSection} and \ref{svDiPASection}.
...
We copy the definitions of leaking cycles, leaking pairs, disclosing cycles, and privacy violating paths from \cite{chadhaLinearTimeDecidability2021} for convenience.
...
This section provides omitted proofs and precise statements of claims made in sections \ref{svDecisionAlgSection} and \ref{svOptimizationSection}.

Note that the technical results of section \ref{svDecisionAlgSection} flow from the approximation algorithm of section \ref{svOptimizationSection}; thus, we order this section such that claims made in \ref{svOptimizationSection} appear first. 
...
\begin{proof}[Proof of proposition \ref{prop:compute_opt_cost}]
...
\begin{proof}[Proof of proposition \ref{approximateSolutionPolyTimeProp}]
...
\begin{proof}[Proof of proposition \ref{prop:approx_opt_are_close}]
    ...
    \begin{align*}
        ...
        &= \max_{\Delta \in [-1, 1]^n} \min_{\gamma, \gamma' \in [-1, 1]^n} \left(\sum_{i \in I} \left(|\Delta_i - \gamma_i| d_i \right) + \sum_{\sigma_i=\texttt{insample}'} |\Delta_i| d_i' \right)\\
        &\geq \max_{\Delta \in [-1, 1]^n} \min_{\gamma, \gamma' \in [-1, 1]^n} \left(\sum_{i \in I} \left(|\gamma_i| - |\Delta_i| \right) d_i  + \sum_{\sigma_i=\texttt{insample}'} |\Delta'_i| d_i' \right)\\
        &= \max_{\Delta \in [-1, 1]^n} \left(- \sum_{i \in I} |\Delta_i| d_i + \sum_{\sigma_i=\texttt{insample}'} |\Delta_i| d_i' + \min_{\gamma, \gamma' \in [-1, 1]^n} \sum_{i \in I}|\gamma_i| d_i \right)\\
...
These results allows us to prove the technical results relating to the privacy constraint graph and a linear time algorithm for privacy (section \ref{svDecisionAlgSection}).
...
The following claim helps to prove proposition \ref{programPrivacyConstraintGraphPathReq}.
...
This section contains full statements of coupling lemmas found in section \ref{mvSection}, as well as any omitted proofs in section \ref{mvSection} (the proof of lemma \ref{mvNotwellformedImpliesNotPrivate} specifically is deferred to appendix C for space). 
